...
\end{equation}
In this thesis work we sometimes work with the chemical potential, and sometimes we set $\mu = 0$, depending on the symmetries that we use \cite{avoiding}. On \cref{fig:hub_scheme} is shown a schematic of the Hubbard model's hopping and interaction.
\begin{figure}[h]
    \begin{center}
        \begin{tikzpicture}
...
Another way to express the two-point correlator is by transforming (\cref{eq:corr_trerm}) to a Euclidean path integral~\cite{discret1,discret2}. We start with representing the two-point correlation function as a path integral
\begin{equation}
